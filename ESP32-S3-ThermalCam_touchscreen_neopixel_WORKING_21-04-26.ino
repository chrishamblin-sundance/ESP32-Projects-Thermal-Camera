...
float MINTEMP = 22;            // Dynamic scale bottom
float MAXTEMP = 65;            // Dynamic scale top
...
void handleTouch() {
  if (touchscreen.touched()) {
    ...
    // --- BUTTON HITBOXES (Center Column) ---
    if (tx > 110 && tx < 190) {
      // Top Button: HOLD Scale
      if (ty > 290 && ty < 350) {
        isPaused = !isPaused;
        drawPauseButton();
        delay(250); 
...
void drawPauseButton() {
  ...
  int bx = 115, by = 300;
  if (isPaused) {
    tft.fillRect(bx, by, 70, 40, RED);
    tft.setTextColor(WHITE);
    tft.drawCentreString("HOLD", 150, 312, 2);
  } else {
    tft.fillRect(bx, by, 70, 40, GREEN);
    tft.setTextColor(BLACK);
    tft.drawCentreString("AUTO", 150, 312, 2);
  }
}

void drawFreezeButton() {
  int bx = 115, by = 380; 
  tft.drawRect(bx, by, 70, 60, WHITE);
  tft.fillRect(bx + 1, by + 1, 68, 58, BLACK);

  if (isFrozen) {
    tft.fillTriangle(bx + 25, by + 15, bx + 25, by + 45, bx + 50, by + 30, GREEN);
  } else {
    tft.fillRect(bx + 22, by + 15, 8, 30, WHITE);
    tft.fillRect(bx + 40, by + 15, 8, 30, WHITE);
  }
}

void drawStaticUI() {
  tft.setTextColor(WHITE, BLACK);
  tft.drawCentreString("Colour", 80, 280, 2);
  tft.drawCentreString("Bright", 220, 280, 2);
